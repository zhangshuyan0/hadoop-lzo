...
public class CustomizedDistributedLzoIndexer extends Configured implements Tool {
  protected static final Log LOG = LogFactory.getLog(DistributedLzoIndexer.class);
  /**
  ...
  private static final String FILE_MIN_SIZE = "file.min.size";
  private static final long DEFAULT_FILE_MIN_SIZE = -1;
  ...
  private void walkPath(Path path, PathFilter pathFilter, List<Path> accumulator) {
    try {
      ...
      if (fileStatus.isDir()) {
        ...
        final long minFileSize = getConf().getLong(FILE_MIN_SIZE,
            DEFAULT_FILE_MIN_SIZE);
        if (fs.getFileStatus(path).getLen() >= minFileSize) {
  ...
  public static void main(String[] args) throws Exception {
    int exitCode = ToolRunner.run(new Configuration(), new CustomizedDistributedLzoIndexer(), args);
